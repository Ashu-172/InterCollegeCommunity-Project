<%-- 
    Document   : Bookmarks
    Created on : Nov 5, 2017, 9:26:30 AM
    Author     : Ashutosh
--%>
...
<html>
    ...
    <body class="hold-transition skin-blue layout-top-nav">
        <%  String Userid = null, Fname = null, Profile_Picture = null, Mail_id = null;
            ...
            int que_id, QUid, likcounts = 0, Anscount = 0;
            String que, QPdate;
            List<Integer> Qids = new ArrayList<Integer>();
            List<Integer> Likes = new ArrayList<Integer>();
            List<Integer> AnswerC = new ArrayList<Integer>();
            ...
            PreparedStatement BMstmt = conn.prepareStatement("select Q_id from bookmarks where U_id=" + Userid);
            ResultSet BMquestion_idrs = BMstmt.executeQuery();
            while (BMquestion_idrs.next()) {
                int id = BMquestion_idrs.getInt(1);
                Qids.add(id);
            }
            for (int i = 0; i < Qids.size(); i++) {
                PreparedStatement stmt = conn.prepareStatement("select * from questions where Q_id="+Qids.get(i));
                    ...
                    <div class="container">
                        ...
                        <div class="collapse navbar-collapse pull-left" id="navbar-collapse">
                            <ul class="nav navbar-nav">
                                ...
                                <li class="active"><a href="Bookmarks.jsp">Bookmarks <span class="sr-only">(current)</span></a></li>
                            </ul>
            ...
            <div class="content-wrapper">
                <div class="container">
                    ...
                    <section class="content-header">
                        <ol class="breadcrumb">
                            <li><a href="Bookmarks.jsp"><i class="fa fa-dashboard"></i> Bookmarks</a></li>
                        </ol>
                    </section>
                    <%
                        for (int i = 0; i < Qids.size(); i++) {
                        ...
                        <div class="col-md-9">
                            ...
                            <div class="box box-widget">
                                ...
                                <div class="box-body">
                                    ...
                                    <a href="AnswerQuestion.jsp?Qid=<%=qid%>"><button type="button" id="Answer<%=i%>" class="btn btn-default btn-xs fa fa-book"> Answer</button></a>
                                    <span class="pull-right text-muted"><%=likecount%> likes - <%=AnswerCount%> Answers</span>
                                </div>
                                <!-- /.box-body -->
                                <div class="box-footer box-comments">
                                    <%  //Answers and answer user data
                                        for (int j = 0; j < Aids.size(); j++) {
                                            //out.println("Answer_id size"+Aids.size());
                                            int A_id = Aids.get(j);
                                            String answer = null, AnsPostdate = null, AnsUserimg = null, AnsUserName = null;
                                            int Ans_Uid = 0;
                                            PreparedStatement stmtansdata = conn.prepareStatement("select * from answers where A_id=" + A_id);
                                            ResultSet rsAnsData = stmtansdata.executeQuery();
                                            while (rsAnsData.next()) {
                                                answer = rsAnsData.getString("Answer_text");
                                                AnsPostdate = rsAnsData.getDate("Create_date").toString();
                                                Ans_Uid = rsAnsData.getInt("Reg_id");
                                            }
                                            PreparedStatement stmtansUserdata = conn.prepareStatement("select * from user_registration where Reg_id=" + Ans_Uid);
                                            ResultSet rsAnsUserData = stmtansUserdata.executeQuery();
                                            while (rsAnsUserData.next()) {
                                                AnsUserName = rsAnsUserData.getString("First_Name") + " " + rsAnsUserData.getString("Last_Name");
                                                AnsUserimg = rsAnsUserData.getString("Profile_Picture");
                                            }
                                    %>
                                    <div class="box-comment">
                                        <!-- User image -->
                                        <img class="img-circle img-sm" src="<%=AnsUserimg%>" alt="User Image">

                                        <div class="comment-text">
                                            <span class="username">
                                                <%=AnsUserName%>
                                                <span class="text-muted pull-right"><%=AnsPostdate%></span>
                                            </span><!-- /.username -->
                                            <%=answer%>
